...
cmd_generate() {
  ...
  for gitignore_file in $GITIGNORE_FILES; do
    if "$GREP_CMD" -q -P '^\s*!' "$gitignore_file"; then
      echo "${YELLOW}⚠️ $(get_relative_path "$gitignore_file" "$BASE_FOLDER") contains exception patterns, will be ignored"
      continue
    fi
  ...
  echo "${YELLOW}✨ Total number of generated files: ${TOTAL_N_GENERATED_FILES} ${DEFAULT}"
}
